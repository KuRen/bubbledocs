...
public class FrontEnd {
    ...
    private BindingProvider putToHandler(String ticket, SDStore replica) {
        BindingProvider bindingProvider;
        Map<String, Object> requestContext;
        bindingProvider = (BindingProvider) replica;
        requestContext = bindingProvider.getRequestContext();
        requestContext.put(RelayClientHandler.REQUEST_PROPERTY, ticket);
    ...
    public void createDoc(DocUserPair docUserPair, String ticket) throws DocAlreadyExists_Exception {
        for (SDStore replica : listOfReplicas) {
            putToHandler(ticket, replica);
            replica.createDoc(docUserPair);
        }
    }
    ...
    // Quorum Consensus protocol not needed 
    public List<String> listDocs(String userId, String ticket) throws UserDoesNotExist_Exception {
        BindingProvider bindingProvider = null;

        List<String> listOfDocuments = null;
        for (SDStore replica : listOfReplicas) {
            bindingProvider = putToHandler(ticket, replica);
    ...
    public void store(DocUserPair docUserPair, byte[] contents, String ticket) throws CapacityExceeded_Exception,
    ...
    public byte[] load(DocUserPair docUserPair, String ticket) throws DocDoesNotExist_Exception, UserDoesNotExist_Exception {
        int maxTag = -1;
        int acks = 0;
        int i;
        boolean ack;
